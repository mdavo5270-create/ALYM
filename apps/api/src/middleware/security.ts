...
/** Never leak stack traces / internal details in production */
...
/**
 * Protect webhooks (Stripe, Resend, etc.) with HMAC signature.
 * Header default: x-signature = hex hmac sha256 of raw body.
 */
export function verifyWebhookSignature(
  ...
  return (req: Request, res: Response, next: NextFunction) => {
    ...
    if (!secret) {
      if (isProd()) return res.status(503).json({ error: 'Webhook non configuré' });
      return next();
    }
...
/** Reject unexpected multipart / large uploads until explicitly supported */
export function rejectUploads(req: Request, res: Response, next: NextFunction) {
  const ct = req.headers['content-type'] || '';
  if (ct.includes('multipart/form-data')) {
    return res.status(415).json({ error: 'Upload non autorisé' });
  }
  next();
}
