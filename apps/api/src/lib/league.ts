...
export async function ensureLeagueForTeam(teamId: number, teamName: string, season = 1) {
...
function formPush(form: string, result: 'W' | 'D' | 'L') {
  const next = `${form}${result}`.slice(-5);
  return next;
}

/** Met à jour le joueur + simule une journée pour les NPC */
export async function applyMatchToLeague(
  teamId: number,
  result: 'W' | 'D' | 'L',
  goalsFor: number,
  goalsAgainst: number
  ...
  for (const npc of npcs) {
    const r = Math.random();
    const res: 'W' | 'D' | 'L' = r < 0.38 ? 'W' : r < 0.62 ? 'D' : 'L';
