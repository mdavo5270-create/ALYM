...
export type EventPriority = 'urgent' | 'action' | 'important' | 'fyi';
...
export type EventChoice = {
  id: string;
  label: string;
  /** codes de conséquence appliqués dans applyConsequences */
  effects: string[];
};
...
export type CareerEventPayload = {
  id: string;
  type: EventType;
...
function bestPlayer(team: TeamSnap) {
...
export function buildCandidateEvents(
  ...
  match?: { result: string },
  rng = Math.random
): Omit<CareerEventPayload, 'id' | 'status' | 'createdAt'>[] {
  const played = team.wins + team.draws + team.losses;
  const candidates: Omit<CareerEventPayload, 'id' | 'status' | 'createdAt'>[] = [];
...
export async function tickCareerEvents(
  teamId: number,
  match?: { result: string },
  rng = Math.random
): Promise<CareerEventPayload[]> {
  ...
  const created: CareerEventPayload[] = [];
...
function rowToPayload(row: {
  ...
  return {
    ...
    options: JSON.parse(row.optionsJson) as EventChoice[],
    status: row.status as CareerEventPayload['status'],
...
export async function listEvents(teamId: number, status?: string) {
...
export async function getPendingEvent(teamId: number): Promise<CareerEventPayload | null> {
  ...
  const weight: Record<string, number> = { urgent: 0, action: 1, important: 2, fyi: 3 };
  all.sort(
...
async function applyEffects(
  ...
  const bumpPlayer = async (id: number, delta: number, fields?: string[]) => {
    const p = team.players.find((x: { id: number }) => x.id === id);
    if (!p) return;
  ...
  for (const effect of effects) {
    ...
    switch (effect) {
      ...
      case 'raise_salary':
        if (playerId) {
          const p = team.players.find((x: { id: number }) => x.id === playerId);
          if (p) {
...
export async function resolveCareerEvent(
  teamId: number,
  eventId: string,
  choiceId: string
): Promise<{ event: CareerEventPayload; result: ApplyResult }> {
  const row = await prisma.careerEvent.findFirst({
    where: { id: eventId, teamId },
  });
  if (!row) throw new Error('Événement introuvable');
  if (row.status !== 'pending') throw new Error('Événement déjà traité');

  const options = JSON.parse(row.optionsJson) as EventChoice[];
  const choice = options.find((o) => o.id === choiceId);
  if (!choice) throw new Error('Choix invalide');

  const context = row.contextJson ? (JSON.parse(row.contextJson) as EventContext) : {};
...
export function toLegacyUnexpectedShape(ev: CareerEventPayload | null) {
  if (!ev) return null;
  return {
    id: ev.id,
