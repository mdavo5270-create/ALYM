...
/** Jobs suggestions for the human manager (vacant or shaky posts) */
router.get('/jobs', async (req, res) => {
  ...
  const playerRep = Math.min(90, 40 + team.wins * 3 + Math.floor(team.budget / 50000));
  ...
  const jobs = clubs
    .map((c) => {
      ...
      return {
        ...
        likelihood: score >= 55 ? 'élevée' : score >= 40 ? 'moyenne' : 'faible',
      };
    })
    .filter(Boolean)
...
router.post('/jobs/:clubId/apply', async (req, res) => {
  const teamId = Number((req.params as Record<string, string>).teamId);
  const clubId = Number((req.params as Record<string, string>).clubId);
  const team = await prisma.team.findFirst({
    where: { id: teamId, userId: req.user!.userId },
  });
  if (!team) return res.status(404).json({ error: 'Équipe introuvable' });
  ...
  await prisma.message.create({
    data: {
      ...
      title: accepted ? `Candidature acceptée — ${club.name}` : `Candidature refusée — ${club.name}`,
      content: accepted
        ? `Le conseil de ${club.name} retient votre profil (compatibilité ${Math.round(score)}). Suite narrative en carrière avancée.`
        : `${club.name} privilégie un autre profil pour le moment (score ${Math.round(score)}).`,
    },
  });
  ...
  await prisma.managerMarketEvent.create({
    data: {
      ...
      detail: `score ${Math.round(score)} · teamId ${teamId}`,
    },
  });
