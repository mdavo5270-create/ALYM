...
router.post('/challenges/start', async (req, res) => {
  ...
  const team = await owned(req.user!.userId, teamId);
  if (!team) return res.status(404).json({ error: 'Équipe introuvable' });
  if (team.challengeId) return res.status(400).json({ error: 'Un défi est déjà actif' });
...
router.post('/training/:playerId', async (req, res) => {
  const teamId = Number((req.params as Record<string, string>).teamId);
  const playerId = Number((req.params as Record<string, string>).playerId);
  const parsed = z
...
export async function applyTrainingGains(teamId: number) {
  ...
  for (const p of players) {
    ...
    if (p.trainingPlan === 'attacking') bump.shot = Math.min(99, p.shot + 1);
      ...
      const keys = ['speed', 'pass'] as const;
      const k = keys[Math.floor(Math.random() * keys.length)];
      bump[k] = Math.min(99, (p as any)[k] + 1);
    }
...
router.post('/loan/:playerId', async (req, res) => {
  ...
  const player = await prisma.player.findFirst({ where: { id: playerId, teamId } });
  if (!player) return res.status(404).json({ error: 'Joueur introuvable' });
  if (player.onLoan) return res.status(400).json({ error: 'Déjà en prêt' });
...
router.post('/loan/:playerId/recall', async (req, res) => {
  ...
  if (!player) return res.status(404).json({ error: 'Prêt introuvable' });
