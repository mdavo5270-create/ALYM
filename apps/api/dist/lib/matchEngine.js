...
export function strengthFromPlayers(players) {
    if (players.length === 0)
        return { attack: 50, midfield: 50, defense: 50, gk: 50 };
...
export function simulateMatch(home, away, homeBonus = 1.08) {
    ...
    const possessionHome = clamp(Math.round(48 + (home.midfield - away.midfield) * 0.6 + (Math.random() * 8 - 4)), 35, 68);
    return {
        homeScore,
        awayScore,
        result,
...
export function withTimeline(sim, homeName, awayName) {
    return {
        ...sim,
...
export function randomOpponentStrength() {
