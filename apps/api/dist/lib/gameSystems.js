...
export function defaultBoardObjectives(wins = 0, budget = 200000) {
    return [
...
export function computeJobSecurity(objectives, form) {
    let score = 55;
...
export function rollUnexpectedEvent(rng = Math.random) {
    if (rng() > 0.35)
        return null; // ~35% chance after a match
    const pool = [
        {
            id: 'injury_star',
            category: 'player',
