...
function visionFitScore(clubVision, managerVision) {
    if (clubVision === managerVision)
        return 100;
    const compat = VISION_COMPAT[clubVision] || [];
    if (compat.includes(managerVision))
        return 65;
    return 20;
}
/**
 ...
 */
export function recruitmentScore(club, manager) {
    ...
    const nationFit = manager.nation && club.nation && manager.nation === club.nation
        ? 90
...
export async function ensureManagerMarketSeed() {
    ...
    for (const m of MANAGER_SEED) {
        managers.push(await prisma.aiManager.create({
            data: {
                name: m.name,
                nation: m.nation,
...
export async function tickManagerMarket(playerTeamId) {
    ...
    const assignments = matchClubsToManagers(stillVacant.map((c) => ({
        id: c.id,
        name: c.name,
        nation: c.nation,
        reputation: c.reputation,
    ...
    for (const a of assignments) {
        ...
        await logEvent('hired', club.name, mgr.name, `${mgr.name} nommé à ${club.name} (score ${a.score.toFixed(1)}, ${mgr.preferredVision}).`);
