...
export async function writeChronicle(teamId, data) {
...
export async function listChronicle(teamId, opts) {
...
export async function seasonReview(teamId, season = 1) {
    ...
    const scored = entries.map((e) => {
        let score = 1;
        if (e.tone === 'triumph' || e.tone === 'turning')
            score += 3;
        if (e.tone === 'setback' || e.tone === 'tension')
            score += 2;
        if (e.type === 'match' && e.tone === 'triumph')
            score += 1;
        if (e.type === 'event')
            score += 1;
        return { ...e, score };
    ...
    const narrative = team.wins >= team.losses + 2
...
export function matchChronicleText(teamName, opponent, homeScore, awayScore, result) {
    if (result === 'W') {
        return {
            tone: 'triumph',
