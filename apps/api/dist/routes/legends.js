...
router.get('/', async (req, res) => {
    ...
    const list = LEGENDS.map((l) => {
        const unlocked = l.unlock === 'challenge_won'
            ? hasChallengeWin
...
router.post('/recruit/:code', async (req, res) => {
    ...
    const codes = new Set(team.achievements.map((a) => a.achievementCode));
    const unlocked = legend.unlock === 'challenge_won'
        ? team.wins >= 3 || codes.has('challenge_won')
        : codes.has(legend.unlock) || (legend.unlock === 'first_win' && team.wins >= 1);
    if (!unlocked)
        return res.status(403).json({ error: 'Légende non débloquée' });
    const cost = 80000;
    if (team.budget < cost)
        return res.status(400).json({ error: 'Budget insuffisant (£80,000)' });
