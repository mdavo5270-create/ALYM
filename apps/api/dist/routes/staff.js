...
router.post('/hire', async (req, res) => {
    ...
    if (!def)
        return res.status(404).json({ error: 'Profil staff inconnu' });
    const team = await owned(req.user.userId, teamId);
    if (!team)
        return res.status(404).json({ error: 'Équipe introuvable' });
    const already = await prisma.staffMember.findFirst({ where: { teamId, role: def.role } });
    if (already)
        return res.status(400).json({ error: 'Poste déjà pourvu' });
...
router.post('/fire/:staffId', async (req, res) => {
    ...
    if (team.budget < severance)
        return res.status(400).json({ error: `Indemnité £${severance} requise` });
